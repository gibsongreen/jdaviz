    ...
    * :meth:`export_extract_spectrum`
    ...
    def _update_interactive_extract(self, event={}):
        if 'extract' not in self.marks:
            ...
            # NOTE: this logic will probably be moved by auto-extraction when importing a 2D
            # spectrum by copying logic from the parser now that uses a clone of the plugin
            # to create an extraction and moving that to the new loader (potentially with a
            # checkbox to disable)
            self.app._on_new_viewer(NewViewerMessage(SpecvizProfileView, data=None, sender=self.app),  # noqa
